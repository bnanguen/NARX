...
def predict_dirmo(mor, scaler, U_test, d, na, nb,
                  block_size, y_mean, y_std,
                  clip_sigma=4.0, n_out=2):
    ...
    while k < Nts:
        ...
        for h in range(1, block_size + 1):
            ...
            if tk >= Nts:
                break
            for j in range(n_out):
                Y_hat[tk, j] = np.clip(block[j * block_size + (h - 1)],
                                        lo[j], hi[j])
        k += block_size
...
def compare_block_sizes(Y_tr, U_tr, Y_ts, U_ts, d, na, nb, base_model,
                        block_sizes=(1, 2, 5, 10, 20, 50),
                        n_out=2, clip_sigma=4.0):
    ...
    for s in block_sizes:
        mor, sc, ym, ys = fit_dirmo(Y_tr, U_tr, d, na, nb,
                                     block_size=s, base_model=base_model,
                                     n_out=n_out)
        Yh = predict_dirmo(mor, sc, U_ts, d, na, nb,
                           block_size=s, y_mean=ym, y_std=ys,
                           clip_sigma=clip_sigma, n_out=n_out)
...
def plot_dirmo_results(results, Y_ts, block_sizes, label=''):
    ...
    ax.set_xlabel('block size s'); ax.set_ylabel('NMSE')
    ...
    for j in (0, 1):
        ...
        a.plot(results[best]['Y_hat'][:, j], color='steelblue',
                lw=0.8, alpha=0.85, label=f's={best}')
        a.set_title(f'y{j+1} (best s={best}, NMSE={results[best]["nmse"]:.3f})')
        a.set_xlabel('k'); a.legend(fontsize=7); a.grid(alpha=0.25)

        a = fig.add_subplot(gs[j, 2])
        a.plot(Y_ts[:, j], color='black', lw=0.8, label='true')
        a.plot(results[worst]['Y_hat'][:, j], color='crimson',
                lw=0.8, alpha=0.85, label=f's={worst}')
        a.set_title(f'y{j+1} (worst s={worst}, NMSE={results[worst]["nmse"]:.3f})')
        a.set_xlabel('k'); a.legend(fontsize=7); a.grid(alpha=0.25)

    plt.suptitle(f'DIRMO {label}'); plt.tight_layout()
    return fig

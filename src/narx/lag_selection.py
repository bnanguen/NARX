...
class NARXFeatureRidge(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        self.model_ = Pipeline([('sc', StandardScaler()),
...
def grid_search(Y, U, d_range, na_range, nb_range,
                proxy_models=None, label='', n_splits=5, verbose=True):
    ...
    for name, model in proxy_models.items():
        ...
        for i, (d, na, nb) in enumerate(cands, 1):
            s = cv_nmse_nonlinear(Y, U, d, na, nb, proxy_model=model,
                                   n_splits=n_splits)
            rows.append((d, na, nb, s))
            if s < best_score:
                best_score, best_triple = s, (d, na, nb)
            if verbose and (i % 10 == 0 or i == total):
                print(f'    {i}/{total}  best so far: {best_triple} '
                      f'NMSE={best_score:.3f}')
    ...
    if len(top) == 1:
        ...
        def avg(t):
            xs = [next(s for dd, nn, mm, s in all_results[name]
                       if (dd, nn, mm) == t)
    ...
    if verbose:
        print(f'\n  [{label}] majority-vote pick: d={best[0]}, '
              f'na={best[1]}, nb={best[2]}')
...
def plot_proxy_comparison(all_results, label='', d_fixed=0):
    ...
    fig, axes = plt.subplots(1, len(models),
                              figsize=(5 * len(models), 4), sharey=True)
    if len(models) == 1:
        axes = [axes]
    ...
    for ax, name in zip(axes, models):
        ...
        lookup = {(d, na, nb): s for d, na, nb, s in all_results[name]
                                       if d == d_fixed}
        ...
        im = ax.imshow(mat, cmap='YlOrRd_r', aspect='auto',
                       vmin=vmin, vmax=vmax)
        ax.set_xticks(range(len(nb_vals))); ax.set_xticklabels(nb_vals)
        ax.set_yticks(range(len(na_vals))); ax.set_yticklabels(na_vals)
        ax.set_xlabel('nb')
        if name == models[0]:
            ax.set_ylabel('na')
